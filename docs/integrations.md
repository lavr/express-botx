...
## Mattermost Bot API: мост для систем мониторинга

Многие системы мониторинга и алертинга умеют доставлять уведомления в Mattermost,
но не умеют произвольный вебхук с заголовками: канал у них настраивается базовым
URL, токеном бота и идентификатором чата, после чего запросы уходят в Mattermost
Bot API. Этот приёмник принимает такие запросы и превращает их в сообщения
eXpress, так что источник настраивается штатно, как на обычный Mattermost.

У способа есть практическое преимущество перед generic-вебхуком: токен уходит
заголовком `Authorization: Bearer`, а не в составе URL, поэтому не попадает в
access-логи ingress и шлюза. Некоторые системы мониторинга секреты в
query-параметрах прямо запрещают.
...
| `PUT` | `/api/v1/mattermost/api/v4/posts/{post_id}` | обновление ранее отправленного |

Отправители обычно строят конечный URL, дописывая `api/v4/posts` к настроенному
базовому URL. Поэтому достаточно указать в настройках канала
...
Пост Mattermost, в котором содержимое обычно лежит во вложении, а `message`
остаётся пустым:
...
```json
{
  ...
  "props": {"attachments": [{
    ...
    "text": "node-1 at 94%",
    "title_link": "https://monitoring.example/alerts/42",
    "fields": [
...
`fields[]` как `title: value`, затем `title_link`, обычно ссылку на алерт.
Непустой `message` идёт впереди вложений. `actions` игнорируются: адаптер не
преобразует их в кнопки eXpress, подтверждать алерт придётся в интерфейсе
источника.

Первая строка каждого вложения получает эмодзи по состоянию алерта. Состояние
берётся **только из полей `fields[]`**, по именам `Status` и `Severity`. Это
конвенция, которую ожидает адаптер, а не требование Mattermost: у известных нам
отправителей эти поля приходят сырыми значениями. `color` не используется: это те же данные,
спроецированные в несколько фиксированных цветов с потерями, из-за чего разные
состояния сливаются в один цвет. Вложение без `Status` и `Severity` доставляется
...
`id` это BotX `sync_id` доставленного сообщения. Отправитель обычно сохраняет
его и позже присылает обратно в `PUT`. `channel_id` возвращается ровно тем же,
что пришёл: клиенты Mattermost сверяют его с запрошенным и при расхождении
помечают доставку как ушедшую не в тот чат.
...
обновление которой обречено: отправителю нечего будет прислать в `PUT`, а
автоматического отката на отправку нового сообщения у таких интеграций обычно
нет.
...
именно такого поведения ожидает от Mattermost отправитель, когда алерт
подтверждён или зарезолвен.
...
- **Статус сообщения ничего не подсвечивает.** Шлюз выводит его из состояния и
  ...
  принимает, хотя в pybotx этого поля нет). Но визуального эффекта у него не
  обнаружено: в проверенном клиенте eXpress сообщение со `status: error`
  выглядит так же, как со `status: ok`, и правка статуса вида не меняет. Официальный SDK
  ...
  теле сообщения, как это делают встроенные шаблоны соседних приёмников.
- **Скоупированный ключ не может редактировать.** В текущей реализации нет
  проверки принадлежности `post_id` чату, поэтому применить скоуп ключа не к
  чему: ключ, ограниченный чатом A, мог бы передать `channel_id: A` и `post_id`
  чужого сообщения в чате B. Шлюз отвечает таким ключам **403** до вызова редактора.
  Используйте для такого канала ключ **без** `chats:`. Обратите внимание, что
  `channel_id` на стороне отправителя авторизацию не заменяет: это адресация,
  и утекший нескоупированный ключ ею не ограничен.
- **Бот для правки выбирается по текущей привязке чата.** Соответствие
  ...
  `PUT`. Если привязку чата к боту поменять между отправкой и обновлением,
  правка уйдёт от другого бота и старое сообщение обновить не удастся.
...
и на несуществующий: валидации нет, приём асинхронный. Шлюз проверяет ответ по
существу (код, разбираемый JSON и `status: ok`) и на любом отклонении отвечает
502, но подтвердить, что правка действительно применилась, ему нечем: несуществующий
`sync_id` вернётся отправителю как успех. Фоллбэка на отправку
нового сообщения нет сознательно: при таймауте правка могла уже примениться, и
повтор оставил бы в чате дубль.
...
server:
  ...
  mattermost:                     # опционально
    default_chat_id: alerts       # если отправитель не прислал channel_id
...
остальных приёмников вебхуков: расходиться в классификации одного и того же
алерта им незачем. `warning_severities` по умолчанию `[warning, warn]`;
`medium` и `low` сознательно не включены, это решение конкретной установки.
Разные источники называют уровни по-разному, поэтому если у вас встречаются
`crit` или `error`, добавьте их в нужный список.
...
В канале типа Mattermost укажите:

1. базовый URL: `https://express-botx:8080/api/v1/mattermost`
2. токен бота: API-ключ express-botx (без `chats:`, см. ограничение выше)
3. идентификатор канала: UUID чата eXpress

Режим работы через Bot API у таких интеграций обычно включается самим фактом
заполнения этих трёх полей.
...
curl -X PUT 'http://localhost:8080/api/v1/mattermost/api/v4/posts/<id>' \
  ...
  -d '{
    ...
    "props": {"attachments": [{
      ...
      "fields": [{"short": true, "title": "Status", "value": "resolved"}]
    }]}
  }'
```
...
- Ключ, привязанный ровно к **одному** чату, шлёт в него `/send` без `chat_id`,
  если чат по умолчанию не задан, а в sync-режиме ещё и когда дефолт ключу не
  разрешён. В async дефолт заранее не проверить: если он задан, выбирается он,
  и при итоговом UUID вне скоупа ответ `403`, свой чат ключа не пробуется. Чат
  берётся так, как он записан в `chats`: алиас с `bot` сохраняет привязку к
  боту, голый UUID в конфиге с несколькими ботами её не даёт.
  Вместе с `allow_query_auth: true` этого хватает отправителю, который умеет
  только `POST <url>` с телом `{"text":"..."}` и без заголовков:
  `POST /api/v1/send?api_key=<ключ>`. `chat_id` в query `/send` не читает, а
  чужой `chat_id` в теле упирается в `403`.
...
клиент. Единственное исключение: скоуп из одного чата служит адресатом `/send`
без `chat_id`, когда дефолт не задан или (в sync) не разрешён; второй чат в
`chats` это отключает.
