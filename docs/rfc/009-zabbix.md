...
Zabbix же сам формирует запрос произвольного формата. Это значит, что JavaScript в Media Type может сразу формировать payload в формате `POST /api/v1/send`, который express-bot уже поддерживает.
...
Вызов `express-bot send` как внешнего скрипта. Подходит для legacy-установок без поддержки Webhook Media Type.
...
| Name | Value |
|------|-------|
| `url` | `http://express-bot:8080/api/v1/send` |
...
if (!resp.ok) {
    throw "express-bot error: " + resp.error;
}
...
**Скрипт** `/usr/lib/zabbix/alertscripts/express-bot.sh`:
...
curl -s -X POST http://express-bot:8080/api/v1/send \
...
```bash
#!/bin/bash
express-bot send --config /etc/express-bot/config.yaml \
  --chat-id "$1" \
  "$2"$'\n\n'"$3"
```
...
- Интеграция с Zabbix API — выходит за рамки express-bot
