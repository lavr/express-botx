...
заголовком `Authorization: Bearer`, а не в составе URL, поэтому не оседает в
access-логах ingress и шлюза и в базе самого отправителя. Некоторые системы
мониторинга секреты в query-параметрах прямо запрещают.
...
Непустой `message` идёт впереди вложений. `actions` игнорируются: кнопки в
eXpress не отрисовываются, подтверждать алерт придётся в интерфейсе источника.

Первая строка каждого вложения получает эмодзи по состоянию алерта. Состояние
берётся **только из полей `fields[]`**: отправители кладут туда `Status` и
`Severity` сырыми значениями. `color` не используется: это те же данные,
...
- **Скоупированный ключ не может редактировать.** `post_id` не несёт чата, в
  ...
  Используйте для такого канала ключ **без** `chats:`; если чат нужно
  ограничить, ограничьте его на стороне отправителя полем `channel_id`.
...
и на несуществующий: валидации нет, приём асинхронный. Поэтому 502 от шлюза на
`PUT` означает только транспортную ошибку или 401; правка, которая не применилась
на стороне платформы, вернётся отправителю как успех. Фоллбэка на отправку
