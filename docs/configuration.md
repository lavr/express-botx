...
server:
  ...
  allow_request_trace: false                # по умолчанию false; true разрешает
                                            # отправителям включать ?trace=1
...
`server.allow_request_trace: true` разрешает отправителям включать логирование
тела запроса маркером `?trace=1` / `X-Botx-Trace`. **По умолчанию `false`**:
отсутствие поля означает, что маркер игнорируется.

Это разрешение на **клиентское принуждение**, а не выключатель логирования тела
вообще: при запущенном с `-vvv` шлюзе тело и отрендеренное сообщение пишутся на
V3 независимо от этой настройки, это решение оператора. Маркер без разрешения
игнорируется молча — запрос доставляется как обычно, ошибки клиент не получает.

Значение читается на старте, поэтому разрешение стоит рестарта (rollout пода)
один раз. Дальше отправитель включает и выключает `?trace=1` правкой своего URL
сколько угодно, без рестартов шлюза. Подробнее про сам режим — в
[docs/integrations.md](integrations.md#отладка-сырой-payload-в-логе-trace1).
...
## Длина сообщения (`max_message_length`, `truncate_suffix`)

BotX ограничивает тело нотификации — в документации Notifications API это 4096
символов. Сообщение сверх лимита апстрим отвергает целиком, и в чат не приходит
ничего: не усечённый текст, а пустота. Поэтому шлюз режет тело сам.

Лимит задаётся **у бота**, рядом с хостом: это свойство конкретной инсталляции
eXpress, и у разных серверов оно может отличаться.
...
bots:
  main:
    ...
    id: ...
    secret: ...
    max_message_length: 4096       # в РУНАХ; 0 выключает обрезку
    truncate_suffix: "…"           # входит в лимит, а не добавляется сверх
```

| Ключ | Не задан | Явное значение |
|---|---|---|
| `max_message_length` | 4096 | `0` выключает обрезку; отрицательное значение — ошибка конфигурации |
| `truncate_suffix` | `…` | пустая строка режет без пометки (обрезку не отключает) |

Правила реза:

- считаются **руны**, не байты — тело почти целиком кириллическое, и байтовый
  рез разрубал бы символы;
- суффикс входит в лимит; суффикс длиннее самого лимита отбрасывается;
- если в последних 10% лимита есть перевод строки, рез идёт по нему — блок
  алерта не рвётся посередине;
- рез никогда не разрубает mention: формы `@{mention:id}`, `@@{mention:id}` и
  `##{mention:id}` целиком переносятся за границу реза;
- записи в массиве `mentions`, чей плейсхолдер **был в исходном теле и пропал
  после реза**, удаляются, иначе запрос ссылался бы на несуществующую разметку.
  Запись, у которой плейсхолдера не было изначально, сохраняется: обрезка не
  меняет смысла явно переданного массива. Если тело не обрезано, массив не
  трогается вовсе.

Если два алиаса ссылаются на один `bot_id`, их `max_message_length` и
`truncate_suffix` должны совпадать: политика выбирается по `bot_id`, и
расхождение сделало бы выбор недетерминированным. Несовпадение — ошибка
конфигурации.

Обрезка применяется ко всем источникам — `/send`, `/alertmanager`, `/grafana`,
`/incidentrelay`, `/gitlab` и CLI, — потому что точка сборки запроса общая. При
каждой обрезке в лог на уровне `-v` идёт строка с исходным и итоговым размером
**в рунах и байтах** и числом отброшенных строк:

```
send: message truncated from 1266 runes (2281 bytes) to 183 runes (333 bytes), 24 lines dropped
```

> Обе единицы пишутся не для красоты. Документация говорит «символов», но какую
> единицу BotX считает на самом деле, по наблюдениям не восстанавливается. Если
> после включения обрезки апстрим продолжит отвергать сообщения, смотрите в этой
> строке **байты**: на русском тексте одна руна — это примерно 1.8 байта, и
> 4096 рун дают около 7.4 КБ.

На старте по каждому боту пишется действующий лимит и его источник:

```
delivery: bot "main" caps messages at 4096 runes (default), truncation suffix "…"
delivery: bot "legacy" message length is not capped (configured)
```
...
- `/send`: `chat_id` из запроса → чат по умолчанию, если ключу он разрешён →
  единственный чат из `chats` API-ключа (как он записан, алиас сохраняет
  привязку к боту) → пустой `chat_id` даёт `400`. В async-режиме чат по
  умолчанию проверить заранее нельзя, поэтому он, если задан, выигрывает. Текст берётся из `message`, а если оно
  пустое, из `text`.
