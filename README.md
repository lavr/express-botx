...
# С конфигом
docker run --rm -v ./express-bot.yaml:/config.yaml lavr/express-bot \
  send --config /config.yaml --chat-id UUID "Hello"

# Запустить HTTP-сервер
docker run --rm -p 8080:8080 -v ./express-bot.yaml:/config.yaml lavr/express-bot \
  serve --config /config.yaml --listen :8080
```
