...
1. **Минимальный MVP** (шаги 1–5, включая 2.5 и 3.5, и 9): JSON mode + raw mode, `--method`, `--input`, `-f`, `-i`, `--timeout`, загрузка конфига, аутентификация, контракт stdout/stderr + тесты.
2. **jq-фильтрация** (шаг 6): добавить `-q` через встроенный `gojq` + тесты.
3. **Типизированные поля** (расширение шага 3): добавить `-F` с автоприведением типов + тесты.
4. **Multipart mode** (расширение шага 3): `--input @file` + `-f` для файловых эндпоинтов + тесты.
5. **Документация** (шаг 10): обновить README.md и docs/commands.md.
